...
const CHAPTERS = [
    { id: 1, name: "Welcome & Resources", pages: 3 },
    { id: 2, name: "Leadership Messages", pages: 11 },
    { id: 3, name: "Health & Wellness", pages: 5 },
    { id: 4, name: "Programs & Classes", pages: 10 }
];
...
function getChapterInfo(page) {
    ...
    for (let chapter of CHAPTERS) {
        if (page <= cumulativePages + chapter.pages) {
            const pageInChapter = page - cumulativePages;
            return {
                chapterId: chapter.id,
                chapterName: chapter.name,
                pageInChapter: pageInChapter,
                totalPagesInChapter: chapter.pages
            };
        }
        cumulativePages += chapter.pages;
    }
    return null;
}

function getLessonByPage(page) {
    return LESSONS.find(lesson => lesson.page === page) || LESSONS[0];
}